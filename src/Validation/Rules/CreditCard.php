...
class CreditCard implements Rule
{
    ...
    public function passes($attribute, $value) {
        ...
        if( ! in_array( $length, preg_split( '/\D+/', $this->cards[ $this->type ]['length'] ) ) ) {
            return false;
        }

        // Check card number prefix
        if( ! preg_match( '/^'.$this->cards[ $this->type ]['prefix'].'/', $number ) ) {
            return false;
        }

        if( $this->cards[ $this->type ]['luhn'] == false ) {
            return true;
        }
